...
for i in range(len(index_name)):
  for ticker in indicies[i]:
    ...
    for i in range(rows):
      ...
      total_spy_vol += spy_data.loc[spy_data.index, 'Volume'].iat[i]
    ...
    for i in range(rows):
      ...
      vol_dev = round(vol_dev, 8)
      w_vol_dev = round(w_vol_dev, 8)
      volume_deviation_values.append(vol_dev)
      w_vol_dev_values.append(w_vol_dev)
      if ((df.loc[df.index, 'Volume'].iat[i] > 1.5 * df.loc[df.index, 'Volume'].iat[i-1])):
